	...
	IF EXISTS (SELECT code from auth_department,pers_person where pers_person.auth_dept_id=auth_department.id and pers_person.pin=new.pin and CAST(auth_department.code as integer) in (2,3,4,12)) THEN
		IF (new.verify_mode_no) != 4 THEN
			IF exists (select masuk from sys_sch_users where sys_sch_users.nik = new.pin and new.event_time<=sys_sch_users.sub_pulang and sys_sch_users.sub_masuk<=new.event_time order by masuk desc limit 1) THEN
				IF exists (select * from acc_transaction_2a,sys_sch_users where pin=new.pin and sub_masuk<=new.event_time and new.event_time<=sub_pulang and sys_sch_users.masuk=acc_transaction_2a.masuk and sys_sch_users.pulang=acc_transaction_2a.pulang and AGE(new.event_time,event_time)<='04:00:00' and (select split_part(dev_alias,'-',1))='OUT' order by event_time desc limit 1) THEN
...
IF (select split_part(new.dev_alias, '-', 1))='OUT' THEN
	...
	IF EXISTS (SELECT code from auth_department,pers_person where pers_person.auth_dept_id=auth_department.id and pers_person.pin=new.pin and CAST(auth_department.code as integer) in (2,3,4,12)) THEN
		IF (new.verify_mode_no) != 4 THEN
			IF exists (select masuk from sys_sch_users where sys_sch_users.nik = new.pin and new.event_time<=sys_sch_users.sub_pulang and sys_sch_users.sub_masuk<=new.event_time order by masuk desc limit 1) THEN
				IF exists (select * from acc_transaction_2a,sys_sch_users where pin=new.pin and sub_masuk<=new.event_time and new.event_time<=sub_pulang and sys_sch_users.masuk=acc_transaction_2a.masuk and sys_sch_users.pulang=acc_transaction_2a.pulang and AGE(new.event_time,event_time)<='04:00:00' order by event_time desc limit 1) THEN
