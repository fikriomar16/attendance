...
const app = angular.module('schInt', []);
app.controller('schInt',($scope,$http) => {
	$scope.closeSch = () => {
	...
	$scope.getToday = () => {
		$http.get(base+'sch_int_tdy').then((res) => {
			table_sch.DataTable().ajax.reload();
			$scope.getSchDate= res.data.date;
		});
	}
	$scope.getYesterday = () => {
		$http.get(base+'sch_int_ytd').then((res) => {
			table_sch.DataTable().ajax.reload();
			$scope.getSchDate= res.data.date;
		});
	}
	$scope.getTomorrow = () => {
		$http.get(base+'sch_int_tmr').then((res) => {
			table_sch.DataTable().ajax.reload();
			$scope.getSchDate= res.data.date;
		});
	}
